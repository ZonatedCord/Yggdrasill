...
`TasselloFissaggioPCB.stl` (PCB mounting dowel, `3D/`) is kept alongside the modular set. Compatibility with the new `Base.stl` mount point is still unconfirmed — verify fit when assembling.
