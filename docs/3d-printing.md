...
| `Base.stl` | Vase base | 135×135×25 | Bayonet joint to lid — clearances unverified empirically |
| `Coperchio.stl` | Lid — carries the PCB mounting points | 143×143×18 | Bayonet joint to base — clearances unverified empirically. **Designed for `BaseChipOnly` only** — see PCB compatibility below |
...
The 3D files in this repo — specifically **`Coperchio.stl`** (the lid, which carries the PCB mounting points) and **`TasselloFissaggioPCB.stl`** — are designed to match the **`BaseChipOnly`** PCB variant (see [docs/hardware.md](hardware.md)).

If you're building with a different variant (`BaseDevKit`, `BaseUSB-C`), the board outline and mounting-hole positions differ, so **the lid's PCB mounting points need to be redesigned in Fusion** (and `TasselloFissaggioPCB.stl` re-checked/re-exported) before printing — they won't line up as-is.
...
`TasselloFissaggioPCB.stl` (PCB mounting dowel, `3D/`) mounts the PCB inside the lid (`Coperchio.stl`), matching `BaseChipOnly`'s mounting holes. Fit against the current `Coperchio.stl` design is still unconfirmed by a physical print — verify when assembling.
